...
local motion_patch = require "motion_patch"
...
local function device_init(driver, device)
    for i = 1, #CLUSTER_ATTR_PAIRS do
        ...
        motion_patch.init_capability_values(device, cluster_id, attribute_ids, nil)
    end
end

local motion_patch_handler = {
  NAME = "Motion Patched Handler",
  lifecycle_handlers = {
    init = device_init,
  },
  capability_handlers = {
    [CheckInInterval.ID] = { [CheckInIntervalCommandName] = motion_patch.checkininterval_capability_handler },
    [FastPollTimeout.ID] = { [FastPollTimeoutCommandName] = motion_patch.fastpolltimeout_capability_handler },
    [IdentifyTime.ID] = { [IdentifyTimeCommandName] = motion_patch.identifytime_capability_handler },
    [DeviceEnabled.ID] = { [DeviceEnabledCommandName] = motion_patch.deviceenabled_capability_handler }
  },
  zigbee_handlers = {
    global = {
      [clusters.PollControl.ID] = {
        [zcl_commands.WriteAttributeResponse.ID] = motion_patch.write_attr_res_handler,
        [zcl_commands.ReadAttributeResponse.ID] = motion_patch.read_attr_res_handler
      },
