...
local button_patch = require "button_patch"
...
local function device_init(driver, device)
    for i = 1, #CLUSTER_ATTR_PAIRS do
        ...
        button_patch.init_capability_values(device, cluster_id, attribute_ids, nil)
    end
end

local button_patch_handler = {
  NAME = "Button Patched Handler",
  lifecycle_handlers = {
    init = device_init,
  },
  capability_handlers = {
    [CheckInInterval.ID] = { [CheckInIntervalCommandName] = button_patch.checkininterval_capability_handler },
    [FastPollTimeout.ID] = { [FastPollTimeoutCommandName] = button_patch.fastpolltimeout_capability_handler },
    [IdentifyTime.ID] = { [IdentifyTimeCommandName] = button_patch.identifytime_capability_handler },
    [DeviceEnabled.ID] = { [DeviceEnabledCommandName] = button_patch.deviceenabled_capability_handler }
  },
  zigbee_handlers = {
    global = {
      [clusters.PollControl.ID] = {
        [zcl_commands.WriteAttributeResponse.ID] = button_patch.write_attr_res_handler,
        [zcl_commands.ReadAttributeResponse.ID] = button_patch.read_attr_res_handler
      },
