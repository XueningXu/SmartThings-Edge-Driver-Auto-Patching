...
local presence_patch = require "presence_patch"
...
local function device_init(driver, device)
    for i = 1, #CLUSTER_ATTR_PAIRS do
        ...
        presence_patch.init_capability_values(device, cluster_id, attribute_ids, nil)
    end
end

local presence_patch_handler = {
  NAME = "Presence Patched Handler",
  lifecycle_handlers = {
    init = device_init,
  },
  capability_handlers = {
    [CheckInInterval.ID] = { [CheckInIntervalCommandName] = presence_patch.checkininterval_capability_handler },
    [FastPollTimeout.ID] = { [FastPollTimeoutCommandName] = presence_patch.fastpolltimeout_capability_handler },
    [IdentifyTime.ID] = { [IdentifyTimeCommandName] = presence_patch.identifytime_capability_handler },
  },
  zigbee_handlers = {
    global = {
      [clusters.PollControl.ID] = {
        [zcl_commands.WriteAttributeResponse.ID] = presence_patch.write_attr_res_handler,
        [zcl_commands.ReadAttributeResponse.ID] = presence_patch.read_attr_res_handler
      },
