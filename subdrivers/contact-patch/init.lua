...
local contact_patch = require "contact_patch"
...
local function device_init(driver, device)
    for i = 1, #CLUSTER_ATTR_PAIRS do
        ...
        contact_patch.init_capability_values(device, cluster_id, attribute_ids, nil)
    end
end

local contact_patch_handler = {
  NAME = "Contact Patched Handler",
  lifecycle_handlers = {
    init = device_init,
  },
  capability_handlers = {
    [CheckInInterval.ID] = { [CheckInIntervalCommandName] = contact_patch.checkininterval_capability_handler },
    [FastPollTimeout.ID] = { [FastPollTimeoutCommandName] = contact_patch.fastpolltimeout_capability_handler },
    [IdentifyTime.ID] = { [IdentifyTimeCommandName] = contact_patch.identifytime_capability_handler },
    [DeviceEnabled.ID] = { [DeviceEnabledCommandName] = contact_patch.deviceenabled_capability_handler }
  },
  zigbee_handlers = {
    global = {
      [clusters.PollControl.ID] = {
        [zcl_commands.WriteAttributeResponse.ID] = contact_patch.write_attr_res_handler,
        [zcl_commands.ReadAttributeResponse.ID] = contact_patch.read_attr_res_handler
      },
